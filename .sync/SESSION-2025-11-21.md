...
**Known Issues (Pre-existing, not related to sync):**
- `InklessConfigsTest` - Tests crash due to TopicBasedRemoteLogMetadataManager configuration issue (missing bootstrap.servers). This is a pre-existing test infrastructure issue.
...
| Type | Count | Example |
|------|-------|---------|
| merge: | 1 | `merge: apache/kafka trunk before 4.3.0-SNAPSHOT` |
| sync(config): | | |
| sync(compile): | | |
| sync(test): | | |
