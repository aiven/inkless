...
import org.apache.kafka.storage.internals.log.{AppendOrigin, AsyncOffsetReadFutureHolder, FetchDataInfo, LocalLog, LogAppendInfo, LogConfig, LogDirFailureChannel, LogLoader, LogOffsetMetadata, LogOffsetSnapshot, LogOffsetsListener, LogSegments, OffsetResultHolder, ProducerStateManager, ProducerStateManagerConfig, RemoteStorageFetchInfo, UnifiedLog, VerificationGuard}
  ...
  class Inkless {
    ...
    def testLastOffsetForLeaderEpochHybridAtSwitchBoundaryUsesFollowerLocalLog(): Unit = {
      ...
      try {
        ...
        partition.makeFollower(
          new LeaderAndIsrRequest.PartitionState()
            .setTopicName(disklessTopicPartition.topic())
            .setPartitionIndex(disklessTopicPartition.partition())
            .setControllerEpoch(0)
            .setLeader(remoteLeaderId)
            .setLeaderEpoch(1)
            .setIsr(Seq[Integer](remoteLeaderId, replicaManager.config.brokerId).asJava)
            .setPartitionEpoch(1)
            .setReplicas(Seq[Integer](replicaManager.config.brokerId, remoteLeaderId).asJava)
            .setIsNew(false),
          new LazyOffsetCheckpoints(replicaManager.highWatermarkCheckpoints.asJava),
      ...
      try {
        ...
        partition.makeFollower(
          new LeaderAndIsrRequest.PartitionState()
            ...
            .setLeader(remoteLeaderId)
            .setLeaderEpoch(1)
            .setIsr(Seq[Integer](remoteLeaderId).asJava)
            .setPartitionEpoch(1)
            .setReplicas(Seq[Integer](replicaManager.config.brokerId, remoteLeaderId).asJava)
            .setIsNew(false),
          new LazyOffsetCheckpoints(replicaManager.highWatermarkCheckpoints.asJava),
