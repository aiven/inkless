...
class ReplicaFetcherThreadTest {
  ...
  def shouldClearPendingEvictionWhenPartitionIsRemoved(): Unit = {
    ...
    // this fetcher. Handing that replacement residency to consolidation would strand the replica
    ...
    assertTrue(f.thread.fetchState(t1p0).isDefined,
      "the replacement residency must keep fetching from the classic leader")
  }
  ...
  def shouldNotEvictWhenIsrShrinksAfterTheBufferDrains(): Unit = {
    ...
    // instead: removing the residency here would leave the replica with no fetcher at all.
    val f = consolidatingFollowerAtSeal(replicaInIsr = true)
