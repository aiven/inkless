...
class InitDisklessLogFlowTest {
  ...
  def testOnMetadataUpdatePartitionChangeRecordWithDisklessFieldsTriggersControlPlaneInit(): Unit = {
    ...
    try {
      ...
      assertEquals(Some(InitState.AwaitingMetadata), ctx.initDisklessLogManager.getInitState(tp))
  ...
  @Test
  def testOnMetadataUpdateFollowerCanAlsoInvokeControlPlaneAfterCommittedMetadata(): Unit = {
    val broker0Ctx = newContext(brokerId = 0)
    val broker1Ctx = newContext(brokerId = 1)
  ...
  private def newContext(brokerId: Int = 0): TestContext = {
    ...
    when(metadataCache.getAllTopics()).thenReturn(Set.empty[String])
    when(metadataCache.getAliveBrokerNodes(any())).thenReturn(Seq(new Node(0, "host0", 9092), new Node(1, "host1", 9093)))
    ...
    val logManager = TestUtils.createLogManager(
      config.logDirs.map(new java.io.File(_)),
    ...
    val metadataPublisher = new BrokerMetadataPublisher(
      ...
      Some(mock(classOf[org.apache.kafka.coordinator.share.ShareCoordinator])),
