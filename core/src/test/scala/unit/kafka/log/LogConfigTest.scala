...
class LogConfigTest {
  ...
  def testDisklessConfigsAtCreation(enable: Boolean): Unit = {
    val kafkaProps = TestUtils.createDummyBrokerConfig()
    val kafkaConfig = KafkaConfig.fromProps(kafkaProps)
    val emptyExistingConfigs = Collections.emptyMap[String, String]()
  ...
  def testDisklessConfigsAtUpdate(): Unit = {
    ...
    val inklessAlreadyEnabled = Map(TopicConfig.INKLESS_ENABLE_CONFIG -> "true").asJava
    ...
    val inklessAlreadyDisabled = Map(TopicConfig.INKLESS_ENABLE_CONFIG -> "false").asJava
