...
object PlaintextProducerSendTest {
  ...
  def quorumAndTimestampConfigProvider: java.util.stream.Stream[Arguments] = {
    ...
    for (groupProtocol <- GroupProtocol.values().map(gp => gp.name.toLowerCase(Locale.ROOT))) {
      ...
      data.add(Arguments.of("kraft", groupProtocol, TopicConfig.MESSAGE_TIMESTAMP_BEFORE_MAX_MS_CONFIG, Long.box(now - fiveMinutesInMs), "diskless"))
      data.add(Arguments.of("kraft", groupProtocol, TopicConfig.MESSAGE_TIMESTAMP_AFTER_MAX_MS_CONFIG, Long.box(now + fiveMinutesInMs), "diskless"))
    }
    data.stream()
  }
}