...
class ReplicaManager(val config: KafkaConfig,
  ...
  def appendRecords(timeout: Long,
    ...
    def classicResponseCallback(classicResult: Map[TopicPartition, PartitionResponse]): Unit = {
      disklessResponsesFuture.whenComplete { case (result, e) =>
        val disklessResult = if (result != null) result.asScala else {
  ...
  def fetchOffset(topics: Seq[ListOffsetsTopic],
    ...
    if (delayedRemoteListOffsetsRequired(statusByPartition)) {
      ...
      val delayedRemoteListOffsets = new DelayedRemoteListOffsets(delayMs, version, statusByPartition, this, _inklessMetadataView.isDisklessTopic, responseCallback)
