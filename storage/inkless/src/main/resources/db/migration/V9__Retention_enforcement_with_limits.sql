...
-- Similar to v1, with the addition of max_batches_per_request to limit the number of batches to be deleted per partition
-- as a way to cope with the performance impact of executing the updates and further deletion of batches.
CREATE FUNCTION enforce_retention_v2(
...
BEGIN
    ...
    LOOP
        ...
        IF l_request.retention_bytes >= 0 OR l_request.retention_ms >= 0 THEN
            ...
            -- Enforce the limit of batches per request if needed
            IF max_batches_per_request > 0 THEN
                -- Find the base offset of the batch that would be the Nth one to delete
                -- This effectively limits how many batches we'll delete in one call
                ...
                SELECT
                    CASE
                        WHEN COUNT(*) <= max_batches_per_request THEN l_base_offset_of_first_batch_to_keep
                        ELSE (SELECT base_offset FROM batches_to_delete ORDER BY base_offset LIMIT 1 OFFSET max_batches_per_request)
                    END INTO l_base_offset_of_first_batch_to_keep
                FROM batches_to_delete;
            END IF;
        END IF;
