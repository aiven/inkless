...
-- Reject fetches below the log start offset with offset_out_of_range, instead of silently
-- returning the first available batch at-or-after the requested offset.
--
-- Previously the lower bound was hardcoded to 0 (r.starting_offset < 0). When log_start_offset
-- had advanced past 0 (deleteRecords, retention, or a consolidating topic whose diskless range
-- begins at the seal), a request for an offset in [0, log_start_offset) passed the error check
-- and the batch query (b.last_offset >= r.starting_offset) returned the first batch at or after
-- the requested offset. For a consumer this silently skips the range [starting_offset,
-- log_start_offset) instead of signalling out-of-range, matching standard Kafka behavior.
--
-- Only the error CASE changes from V8: r.starting_offset < 0 -> r.starting_offset < l.log_start_offset.
-- Once the error is set, the batch CTE (WHERE r.error IS NULL) is skipped, so no batch can leak.
        ...
        requests_with_log_info AS (
            SELECT
                ...
                CASE
                    ...
                    WHEN r.starting_offset < l.log_start_offset OR r.starting_offset > l.high_watermark THEN 'offset_out_of_range'::find_batches_response_error_v1
                    ELSE NULL
                END AS error
            FROM requests r
