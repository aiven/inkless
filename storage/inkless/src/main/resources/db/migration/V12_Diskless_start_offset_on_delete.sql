-- Copyright (c) 2024-2026 Aiven, Helsinki, Finland. https://aiven.io/
CREATE OR REPLACE FUNCTION delete_batch_v1(
...
DECLARE
    ...
    l_batch RECORD;
BEGIN
    -- update the diskless_start_offset first
    FOR l_batch IN
        SELECT *
        FROM batches WHERE batch_id = arg_batch_id
    LOOP
        UPDATE logs
        SET diskless_start_offset = (l_batch.last_offset + 1)
        WHERE topic_id = l_batch.topic_id AND partition = l_batch.partition;
    END LOOP;

    -- delete the batch and mark the file to delete if it's empty
    DELETE FROM batches
...
DECLARE
    ...
    l_diskless_start_offset BIGINT;
BEGIN
    ...
    FOR l_request IN
        ...
        LOOP
            ...
            l_diskless_start_offset = GREATEST(l_converted_offset, l_log.diskless_start_offset);
            ...
            SET log_start_offset = l_converted_offset,
                ...
                diskless_start_offset = l_diskless_start_offset
            WHERE topic_id = l_log.topic_id
              AND partition = l_log.partition;
