 ...
 * @param logStartOffset      The log start offset (boundary where local log ends)
...
public record InitDisklessLogRequest(Uuid topicId,
    ...
    /**
     * Creates a request without producer state entries (for backward compatibility).
     */
    public static InitDisklessLogRequest create(Uuid topicId,
                                                String topicName,
                                                int partition,
                                                long logStartOffset,
                                                long disklessStartOffset,
                                                int leaderEpoch) {
        return new InitDisklessLogRequest(topicId, topicName, partition, logStartOffset,
            disklessStartOffset, leaderEpoch, List.of());
    }

    /**
     * Creates an InitDisklessLogRequest from a UnifiedLog.
     * <p>
     * This method extracts all necessary information from the log including:
     * <ul>
     *   <li>Topic ID and partition information</li>
     *   <li>Log start offset and high watermark (as diskless start offset)</li>
     *   <li>Leader epoch from the leader epoch cache</li>
     *   <li>Producer state entries for idempotency support</li>
     * </ul>
     *
     * @param log         The UnifiedLog to extract information from
     * @param leaderEpoch The current leader epoch for this partition
     * @return An InitDisklessLogRequest containing all the log state
    ...
    public static InitDisklessLogRequest fromUnifiedLog(final UnifiedLog log, final int leaderEpoch) {
        ...
        final List<ProducerStateSnapshot> producerStateEntries = extractProducerState(log);
    ...
    /**
     * Extracts the producer state from the log's ProducerStateManager.
     * <p>
     ...
     *
     * @param log The UnifiedLog to extract producer state from
     * @return A list of ProducerStateSnapshot entries for all active producers
     */
    private static List<ProducerStateSnapshot> extractProducerState(final UnifiedLog log) {
        final Map<Long, ProducerStateEntry> activeProducers = log.producerStateManager().activeProducers();
        ...
        for (final Map.Entry<Long, ProducerStateEntry> entry : activeProducers.entrySet()) {
            ...
            // Each producer can have up to NUM_BATCHES_TO_RETAIN batch metadata entries
