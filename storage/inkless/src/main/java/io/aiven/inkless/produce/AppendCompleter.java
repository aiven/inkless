...
class AppendCompleter {
    ...
    private final BatchCoordinateCache cache;

    public AppendCompleter(ClosedFile file, BatchCoordinateCache cache) {
        this.file = file;
        this.cache = cache;
    }
    ...
    public void finishCommitSuccessfully(List<CommitBatchResponse> commitBatchResponses) {
        ...
        for (int i = 0; i < commitBatchResponses.size(); i++) {
            ...
            if (batchInfo != null) {
                cache.put(commitBatchRequest.topicIdPartition(), batchInfo);
            }
