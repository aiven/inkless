...
public interface BatchCoordinateCache extends Closeable {
    ...
    LogFragment get(TopicIdPartition topicIdPartition);

    LogFragment get(TopicIdPartition topicIdPartition, long offset);

    LogFragment get(TopicIdPartition topicIdPartition, long offset, long highWaterMark, long logStartOffset);

    void put(BatchCoordinate batchCoordinate);

    int invalidatePartition(TopicIdPartition topicIdPartition);

    long size();

    long totalPartitionInvalidations();
}
