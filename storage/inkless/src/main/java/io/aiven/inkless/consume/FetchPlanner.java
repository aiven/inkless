...
public class FetchPlanner implements Supplier<List<FetchPlanner.FetchRequestWithFuture>> {
    ...
    private FetchRequestWithFuture submitSingleRequest(final ObjectFetchRequest request) {
        if (!request.lagging() && !isConsolidationFetch) {
            ...
            // Taken in the registration callback, which the cache runs at the moment the load is handed
            // to the executor: capturing before computeIfAbsent would fold the cache lookup into the queue wait.
            ...
            if (isConsolidationFetch) {
                try {
                    ...
                    if (cached != null) {
                        ...
                        metrics.recordRecentDataRequest();
                        return new FetchRequestWithFuture(request, CompletableFuture.completedFuture(cached), false);
                    }
            ...
            try {
                ...
                metrics.recordLaggingConsumerRejection();
                return new FetchRequestWithFuture(request, CompletableFuture.failedFuture(e), true);
            }
        }
    }
     ...
     *
     * @param cacheBypassing true if the fetch does not go through the object cache, so nothing but
     *                       this request holds what it returns. A hot-path miss is not bypassing:
     *                       {@code computeIfAbsent} registers the entry before the load runs.
     */
    public record FetchRequestWithFuture(
        ObjectFetchRequest request,
        CompletableFuture<FileExtent> future,
        boolean cacheBypassing
    ) {}
}
