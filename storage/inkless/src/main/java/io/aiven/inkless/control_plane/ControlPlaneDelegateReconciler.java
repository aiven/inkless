 ...
 * rather than waiting. {@link #start()} exists to make that rare: the owner calls it during startup
 * so the delegate is normally built before the broker accepts traffic. Callers must still tolerate
 * the failure, which is retriable.
 *
...
class ControlPlaneDelegateReconciler implements Closeable {
    ...
    private void requestReconcile() {
        ...
        try {
            ...
            // Shut down: there is nothing left to reconcile towards.
    ...
    private boolean attempt(final long want, final Actual current) {
        ...
        try {
            ...
            // An unreachable control plane is not an unconfigured one. Retry under a backoff.
