    ...
    CompletableFuture<Map<TopicPartition, PartitionResponse>> add(
        ...
        for (final var entry : entriesPerPartition.entrySet()) {
            ...
            try {
                ...
                brokerTopicStats.topicStats(topicIdPartition.topic()).bytesInPerDisklessTopicTypeRate().mark(records.sizeInBytes());
                brokerTopicStats.allTopicsStats().bytesInPerDisklessTopicTypeRate().mark(records.sizeInBytes());
