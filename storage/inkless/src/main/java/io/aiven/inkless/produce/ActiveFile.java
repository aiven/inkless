...
class ActiveFile {
        ...
        for (final var entry : entriesPerPartition.entrySet()) {
            ...
            try {
                ...
                if (appendInfo.validBytes() <= 0) {
                    ...
                    var validRecords = trimInvalidBytes(topicIdPartition.topicPartition(), records, appendInfo);
                ...
                result.put(topicIdPartition.topicPartition(), CompletableFuture.completedFuture(new PartitionResponse(Errors.forException(e))));
            } catch (RecordValidationException rve) {
                processFailedRecords(topicIdPartition.topicPartition(), rve.invalidException());
                result.put(topicIdPartition.topicPartition(), CompletableFuture.completedFuture(
                ...
                result.put(topicIdPartition.topicPartition(), CompletableFuture.completedFuture(new PartitionResponse(Errors.forException(t))));
            }
        }
    ...
    // Moved from UnifiedLog.
    // This method is being migrated to Java and this is a placeholder for when it becomes available from apache/kafka#19030
    /**
