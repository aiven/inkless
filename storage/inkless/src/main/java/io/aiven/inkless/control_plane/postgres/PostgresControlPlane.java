...
public class PostgresControlPlane extends AbstractControlPlane {
    ...
    @Override
    public GetLogInfoResponse getLogInfo(final Uuid topicId, final int partition) {
        try {
            final GetLogInfoJob job = new GetLogInfoJob(time, jooqCtx, topicId, partition, metrics::onGetLogInfoCompleted);
            return job.call();
        } catch (final Exception e) {
