...
public class PostgresControlPlane extends AbstractControlPlane {
    ...
    public void createTopicAndPartitions(final Set<CreateTopicAndPartitionsRequest> requests) {
        ...
        new TopicsAndPartitionsCreateJob(time, jobsJooqCtx, requests, metrics::onTopicCreateCompleted).run();
    }
    ...
    public void deleteTopics(final Set<Uuid> topicIds) {
        final DeleteTopicJob job = new DeleteTopicJob(time, jobsJooqCtx, topicIds, metrics::onTopicDeleteCompleted);
        job.run();
    }

    @Override
    public List<DeleteRecordsResponse> deleteRecords(final List<DeleteRecordsRequest> requests) {
        final DeleteRecordsJob job = new DeleteRecordsJob(time, jobsJooqCtx, requests, metrics::onDeleteRecordsCompleted);
        return job.call();
    }
    ...
    public List<EnforceRetentionResponse> enforceRetention(final List<EnforceRetentionRequest> requests, final int maxBatchesPerRequest) {
        try {
            final EnforceRetentionJob job = new EnforceRetentionJob(time, jobsJooqCtx, requests, maxBatchesPerRequest, metrics::onEnforceRetentionCompleted);
            return job.call();
    ...
    public List<FileToDelete> getFilesToDelete() {
        try {
            final FindFilesToDeleteJob job = new FindFilesToDeleteJob(time, jobsJooqCtx, metrics::onGetFilesToDeleteCompleted);
            return job.call();
        } catch (final Exception e) {
    ...
    public void deleteFiles(DeleteFilesRequest request) {
        try {
            final DeleteFilesJob job = new DeleteFilesJob(time, jobsJooqCtx, request, metrics::onFilesDeleteCompleted);
            job.run();
        } catch (final Exception e) {
    ...
    public void releaseFileMergeWorkItem(final long workItemId) {
        final ReleaseFileMergeWorkItemJob job =
            new ReleaseFileMergeWorkItemJob( time, workItemId, jobsJooqCtx, metrics::onReleaseFileMergeWorkItemCompleted);
        final var result = job.call();
        switch (result.getError()) {
            case none:
                break;
    ...
    public boolean isSafeToDeleteFile(String objectKeyPath) {
        try {
            final SafeDeleteFileCheckJob job =
                new SafeDeleteFileCheckJob(time, jobsJooqCtx, objectKeyPath, metrics::onSafeDeleteFileCheckCompleted);
            return job.call();
        } catch (Exception e) {
    ...
    public List<GetLogInfoResponse> getLogInfo(final List<GetLogInfoRequest> requests) {
        try {
            ...
            final GetLogInfoJob job = new GetLogInfoJob(time, jobsJooqCtx, requests, metrics::onGetLogInfoCompleted);
            return job.call();
        } catch (final Exception e) {
