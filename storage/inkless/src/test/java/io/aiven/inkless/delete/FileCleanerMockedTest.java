...
class FileCleanerMockedTest {
    ...
    void empty() throws Exception {
        final var cleaner = new FileCleaner(time, controlPlane, storage, OBJECT_KEY_CREATOR, RETENTION_PERIOD);
        when(controlPlane.getFilesToDelete()).thenReturn(List.of());

        cleaner.run();

        verify(storage, times(0)).delete(Set.of());
    }
    ...
    void singleWithinRetention() throws Exception {
        ...
        when(storage.delete(Set.of(objectKey)))
            .thenReturn(CompletableFuture.completedFuture(null));

        cleaner.run();

        verify(storage, times(1)).delete(Set.of(objectKey));
    ...
    void multiple() throws Exception {
        ...
        when(storage.delete(new HashSet<>(objectKeys)))
            .thenReturn(CompletableFuture.completedFuture(null));

        cleaner.run();

        verify(storage, times(1)).delete(new HashSet<>(objectKeys));
