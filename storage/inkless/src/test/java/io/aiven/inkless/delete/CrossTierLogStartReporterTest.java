...
class CrossTierLogStartReporterTest {
    ...
    @Test
    void reportsZeroOffset() {
        // 0 is load-bearing: it is the cross-tier earliest of a freshly-tiered born-consolidated topic
        // whose WAL prune frontier has advanced above 0. The EARLIEST read path returns
        // COALESCE(remote_log_start_offset, log_start_offset), so 0 must reach the control plane and cache;
        // it must NOT be short-circuited like a negative sentinel.
        asConsolidatingDiskless();
        when(controlPlane.advanceCrossTierLogStartOffset(any()))
            .thenReturn(List.of(AdvanceCrossTierLogStartOffsetResponse.success(0)));

        reporter.enqueue(TP0, 0);
        assertThat(reporter.pendingView()).containsExactly(java.util.Map.entry(TIDP0, 0L));

        reporter.run();

        verify(controlPlane).advanceCrossTierLogStartOffset(
            List.of(new AdvanceCrossTierLogStartOffsetRequest(TOPIC_ID, 0, 0)));
        verify(cache).put(TIDP0, 0L);
        assertThat(reporter.pendingView()).isEmpty();
    }
