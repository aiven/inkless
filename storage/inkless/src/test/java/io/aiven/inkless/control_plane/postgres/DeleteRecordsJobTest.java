...
class DeleteRecordsJobTest {
    ...
    void deleteRecordsFromMultipleTopics(final List<Integer> order) {
        ...
        assertThat(DBUtils.getAllLogs(pgContainer.getDataSource())).containsExactlyInAnyOrder(
            new LogsRecord(TOPIC_ID_0, 0, TOPIC_0, 18L, 36L, (long) file2Batch1Size + file3Batch1Size, 0L, null),
            new LogsRecord(TOPIC_ID_0, 1, TOPIC_0, 24L, 24L, 0L, 0L, null),
            new LogsRecord(TOPIC_ID_1, 0, TOPIC_1, 0L, 0L, 0L, 0L, null),
            new LogsRecord(TOPIC_ID_2, 0, TOPIC_2, 0L, 24L, (long) file2Batch2Size + file3Batch3Size, 0L, null)
        );
