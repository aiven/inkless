...
public class FetchPlannerTest {
        ...
        assertBatchPlan(
            Map.of(
                ...
                FindBatchResponse.success(
                    List.of(
                        BatchInfo.of(OBJECT_KEY_A_MAIN_PART, 0, 10, 0, 0, 0, 10, 20, TimestampType.CREATE_TIME),
                        BatchInfo.of(OBJECT_KEY_B_MAIN_PART, 0, 10, 1, 1, 1, 11, 21, TimestampType.CREATE_TIME)
                    ), 0, 2
                )
            ),
            Set.of(
    ...
    public void planOffsetOutOfRange() throws Exception {
        assertBatchPlan(
            Map.of(
                partition0,
                FindBatchResponse.offsetOutOfRange(0, 1),
                partition1,
                FindBatchResponse.success(List.of(
                    BatchInfo.of(OBJECT_KEY_B_MAIN_PART, 0, 10, 0, 0, 0, 11, 21, TimestampType.CREATE_TIME)
                ), 0, 1)
    ...
    public void planUnknownServerError() throws Exception {
        assertBatchPlan(
            Map.of(
                partition0,
                FindBatchResponse.unknownServerError(),
                partition1,
                FindBatchResponse.success(List.of(
                    BatchInfo.of(OBJECT_KEY_B_MAIN_PART, 0, 10, 0, 0, 0, 11, 21, TimestampType.CREATE_TIME)
                ), 0, 1)
