...
- **Correctness-first reads**:
    - `< B0` reads use Kafka's tiered read semantics (RemoteLogManager + RLMM/RSM).
    - **Replica-served tiered reads**: After `tieringComplete`, **any replica** (not just the leader) may serve `< B0` reads from tiered storage, consistent with standard Kafka tiered storage behavior. This enables load distribution and higher availability for historical data.
...
- **Non-transactional diskless data**: Diskless suffix (`>= B0`) supports only non-transactional produce. All transactions must complete before migration.
...
- **Control Plane (CP)**: current Postgres-backed metadata backend (future: TopicBasedBatchCoordinator).
...
- **`B0` (`disklessLogStartOffset`)**
    ...
    - **Implementation note**: brokers cache `B0` lookups from the control plane (TTL ~1s) to keep Fetch/ListOffsets hot paths from hammering the CP; on transient CP failures they continue using the last cached value.
- **`tieringComplete`**
...
- **I2 — Boundary immutability**: `B0` is immutable once established.
...
At trigger time, the leader selects and persists `B0` for each partition.
...
2. **Control plane initialization** (see "Control plane initialization requirements" section below):
    - Initialize diskless log at non-zero offset (B0 = current LEO)
    - Persist producer state snapshot at B0
    - Persist leader epoch metadata at B0
...
2. **Transaction completion**: All ongoing transactions must complete (commit or abort) before `B0` is selected. This prevents splitting transactions across the migration boundary.
3. **Empty topics**: For an empty local log, the broker initializes `B0=0` in the control plane so diskless produce/ListOffsets can start at offset 0. No tiered storage is required.
...
1. **Primary path**: `TopicConfigHandler#updateLogConfig` when `diskless.enable` changes from `false` to `true`
    ...
    - Initiates migration: validation → transaction wait → boundary selection → CP persistence
...
sequenceDiagram
  ...
  Broker->>CP: setDisklessLogStartOffset(tp, B0)
  CP-->>Broker: OK (immutable)
...
- **Actions**:
    ...
    - If `B0 < LocalLog.logStartOffset` (boundary points to already-truncated data): the broker **resets the boundary** to `UNDEFINED` (−1), as the migration state is no longer valid. Re-migration must be triggered manually.
...
When persisting `B0` to the control plane, the leader must also initialize metadata for the **tiered prefix only** (diskless data does not support transactions):

1. **Producer state snapshot**: Capture the producer ID mapping and sequence numbers at `B0` to preserve idempotency state for the tiered prefix (`< B0`)
2. **Leader epoch metadata**: Persist the leader epoch cache state at `B0` for epoch-based truncation and fencing
3. **Last stable offset**: Record the LSO at migration time (same as `B0` since all transactions must complete before migration)
4. **Abort markers**: Ensure any abort markers for transactions ending before `B0` are properly indexed in the tiered prefix

**Important**: This metadata is for preserving the state of the tiered prefix. The diskless suffix (`>= B0`) does NOT support transactional produce or exactly-once semantics.
...
- If the leader crashes after persisting `B0` to the control plane but before updating local state, the next leader (via `applyLocalLeadersDelta`) will read `B0` from the control plane and complete initialization
- If `B0` is already set in the control plane, the operation succeeds (idempotent write)
- If local state shows migration complete but control plane has no `B0`, the leader re-initializes (control plane is source of truth)
...
This is used as the gate to allow **non-leader replicas** to serve tiered prefix reads from remote storage. **Replica membership requirement**: Only brokers that are members of the partition's replica set can serve reads. Serving from arbitrary non-replica brokers is explicitly not supported in this design.
...
#### 2. **Config changed, B0 initialization in progress**
- **Duration**: Milliseconds to seconds (transaction wait + segment roll + flush + CP write)
- **Produce requests**:
    - Leader: **BLOCKS** until B0 is persisted to control plane
    - Followers: Return `NOT_LEADER_OR_FOLLOWER` (should not receive produce requests)
- **Fetch requests**: Continue serving from classic/tiered path (B0 not yet visible)
- **Crash safety**: If leader fails during initialization, new leader will query CP for B0 and complete migration (idempotent)
...
**Error code when B0 not initialized**: `NOT_LEADER_OR_FOLLOWER` (forces client to retry and refresh metadata). Default `UNKNOWN_TOPIC_OR_PARTITION` should be mapped to `NOT_LEADER_OR_FOLLOWER` for diskless topics.
...
- **Cache invalidation**: Never (B0 is immutable once set)
- **TTL for control plane queries**: ~1s to avoid hammering CP on hot paths
...
Diskless topics require Inkless shared state (CP + object store client) to be configured on the broker.

- If a diskless fetch arrives on a broker that does not have Inkless enabled, the broker returns **`KAFKA_STORAGE_ERROR`** for the affected partitions (not an empty response).
...
**Replica membership requirement**: Only brokers that are members of the partition's replica set can serve reads. Arbitrary non-replica brokers cannot serve data.
...
- Follower fetches that would involve **diskless suffix** reads (`>= B0`) do not replicate bytes.
    - To keep ISR stable in RF>1 clusters, the Kafka leader responds to follower fetches in the diskless region with **`Errors.NONE` + empty records**, and caps `highWatermark/lastStableOffset` at `B0`.
...
- Control-plane failures are mapped deterministically:
    - transient CP unavailability/timeouts → `REQUEST_TIMED_OUT`
    - fatal/unknown CP failures → `KAFKA_STORAGE_ERROR`
    - missing topic/partition identity → `UNKNOWN_TOPIC_OR_PARTITION`
- For hybrid timestamp queries, replicas perform a **dual-search** (tiered prefix + diskless suffix) and return the candidate with the **smallest offset**. The implementation enforces that the **suffix** candidate must be `>= B0` (otherwise `KAFKA_STORAGE_ERROR`) and the **prefix** candidate must be `< B0` (otherwise it is treated as a storage error / falls back). Non-leader replicas can serve these queries once `tieringComplete` is satisfied; otherwise they return `NOT_LEADER_OR_FOLLOWER` to force a retry to the leader.


All ListOffsets routing is implemented in `ReplicaManager.fetchOffset(...)`.

### Diskless region (CP authoritative, but hybrid-correct)
...
- For pure diskless topics (B0=0), CP can serve EARLIEST.

### EARLIEST on non-leader replicas (after tieringComplete)

Once `tieringComplete` is true, non-leader replicas may serve **EARLIEST** by returning the **earliest remotely available offset** for the tiered prefix (minimum `startOffset` among `COPY_SEGMENT_FINISHED` remote segments). This supports any-replica routing for metadata-rewritten clients.
...
flowchart TD
  ...
  D -- Yes --> F{"Hybrid? (B0 > 0)"}
  F -- No --> G[Pure diskless: CP EARLIEST]
  ...
  M --> O["Diskless suffix timestamp lookup (>=B0 via CP)"]
...
This migration design relies on metadata rewriting for **client routing** (primarily Fetch/ListOffsets) for legacy clients.
...
- For diskless-suffix operations served from Inkless/CP, the serving broker does not require a local log/epoch match.
...
- Non-replica brokers are **never** advertised as leaders, ensuring only replicas serve reads.
...
The control plane stores the authoritative partition boundary and migration metadata:

### Core tables and columns

- `diskless_log_start_offset` (partition boundary `B0`)
- `producer_state_snapshot` (producer IDs, sequences, epochs at `B0` - for tiered prefix only)
- `leader_epoch_checkpoint` (epoch boundaries at migration time)
- `last_stable_offset_at_migration` (equals `B0` - transactions must complete before migration)
...
- `V14__Add_migration_metadata.sql` (NEW) - adds producer state and epoch metadata

### Functions

- `initializeDisklessLog(topicId, partition, boundary, producerState, epochs)` - atomic migration initialization
...
- **Critical**: Migration with ongoing transactions waits for completion (all transactions must commit or abort)
- Migration correctly uses `lastStableOffset` as `B0` when transactions are pending (ensuring no transaction spans the boundary)
- Abort markers for transactions ending before `B0` are properly indexed in the tiered prefix
- Producer state snapshot at `B0` is correctly persisted for the tiered prefix
- **After migration**: Diskless suffix (`>= B0`) does NOT support transactional produce - only non-transactional writes are allowed
...
- Replace Postgres CP with TopicBasedBatchCoordinator.
