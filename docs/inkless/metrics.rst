...
LaggingConsumerRequestRate           Rate of cold-path requests (bypass the cache) per second. Under the consumer metrics group these are lagging-consumer fetches; under the consolidation group (ConsolidationFetchMetrics) these are consolidation fetches, including cache-hit reuse of consumer-cached data.
