...
FetchRecentObjectBytes               Bytes of object data one fetch request held in ranges that live in the object cache, summed over the request. Recorded only for requests that held at least one, including the request that loaded it, since the cache owns the entry either way.                                                                                                
