...
|                                                                 | `ConsolidationRemotePrefixUnknown`                          | 1 while a WAL-gap fetch is waiting on RLMM to decide `OFFSET_MOVED` vs never-tiered `OFFSET_OUT_OF_RANGE`; 0 once decided. Broker aggregate is the count of waiting partitions. A leadership transition or fetcher re-arm can dip the gauge to 0 while the partition is still waiting; the next backed-off fetch restores 1. Register and fetcher removal bump a generation. `DisklessLeaderEndPoint.fetch` captures that generation before the delayed fetch parks so an in-flight mark cannot restore the latch after a bump. Alert on a sustained non-zero value after `__remote_log_metadata` catch-up, not on the instantaneous reading. |
