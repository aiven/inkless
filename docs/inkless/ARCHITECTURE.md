...
At startup the broker waits for that first build to finish before it serves any request,
so nothing races it. The wait is bounded, and an unconfigured or unreachable control plane
ends it early rather than holding up startup.
