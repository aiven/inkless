...
| `DisklessWithoutRemoteStorageCount` | Diskless topics where `remote.storage.enable` is `false` or unset. These topics do not consolidate until `remote.storage.enable=true` is set, with consolidation on. Refreshed on metadata snapshot (default: every hour or 20 MB of metadata records) |
