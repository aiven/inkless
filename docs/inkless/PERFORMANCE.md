...
Consumer fetch performance is heavily influenced by caching. Inkless implements a distributed cache within each availability zone (rack):

- **Cache Hits**: When data is already cached (from recent producer writes or previous consumer reads), fetch latencies are significantly lower - typically in the range of 10-50ms depending on inter-broker network latency
...
The cache is distributed within each availability zone, meaning brokers in the same AZ share cached data. This has important implications for cost optimization:
...
- Producers write to brokers in their AZ, populating the local cache
- Consumers read from the same AZ, benefiting from cache hits
...
- Each AZ maintains its own cache
- A consumer in AZ-A reading data produced in AZ-B will incur a cache miss
...
See [CLIENT-BROKER-AZ-ALIGNMENT.md](CLIENT-BROKER-AZ-ALIGNMENT.md) for detailed AZ configuration guidance.
...
- Consumers primarily fetch from brokers in the same availability zone
- Cache hits are served locally within the same AZ
