...
4. Track progress in .inkless-sync/SESSION-YYYY-MM-DD.md using the template
...
2. Create .inkless-sync/ directory
3. Copy MAIN-SYNC-SESSION-TEMPLATE.md to .inkless-sync/SESSION-$(date +%Y-%m-%d).md
...
Update the session file in .inkless-sync/ with the conflict summary.
...
Update the session file in .inkless-sync/ with resolutions.
```
...
Update the session file in .inkless-sync/ with error log and fixes.
```
...
Update the session file in .inkless-sync/ with test log and fixes.
```
...
3. Update the session file in .inkless-sync/ with final status

4. Archive the session file:
   mv .inkless-sync/SESSION-*.md inkless-sync/sessions/

5. If successful, provide PR description draft.
```
...
Read the session file in .inkless-sync/ for full context and continue from where we left off.
```
...
3. **Document everything** - Keep the session file in .inkless-sync/ updated
