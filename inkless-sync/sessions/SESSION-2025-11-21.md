...
### Java Consumer Tests: Add Diskless Topic Support

**Context:** Upstream migrated Scala consumer tests to Java at `clients/clients-integration-tests/src/test/java/org/apache/kafka/clients/consumer/PlaintextConsumerTest.java` (75 tests, 1785 lines). The Scala `PlaintextConsumerTest.scala` now contains only 2 upstream tests.

**Issue:** The Java tests only cover `classic` and `consumer` (async) group protocols, but NOT diskless topics. The Scala test infrastructure had diskless support via `@Tag("inkless")` and `GroupProtocolAndMaybeTopicTypeProvider`.

**Options:**
1. **Modify Java test infrastructure** - Add `DisklessMode` support to `@ClusterTest`/`ClusterInstance`, parameterize with topicType (large divergence from upstream)
2. **Create `InklessPlaintextConsumerTest.java`** - New file with diskless-specific tests using inkless containers (simpler, contained)
3. **Keep Scala tests for diskless** - Restore inkless Scala tests (tests in two languages)

**Recommendation:** Option 2 - Create focused Java test class for diskless consumer scenarios.

**Location:** `clients/clients-integration-tests/src/test/java/org/apache/kafka/clients/consumer/`
